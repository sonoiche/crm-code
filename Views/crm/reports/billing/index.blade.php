	...
	<div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="box-body" id="report-filter">
				{!! Form::open(['url'=>'client/crm/reports/billing','id'=>'reportForm','class'=>'role-form']) !!}
				<div class="row">
					<div class="col-lg-12">
						<div class="form-group">
							<input type="text" name="customer_id" id="customer_id" class="form-control">
							<small class="error">Leave blank to generate all customer</small>
						</div>
					</div>
				</div>
				<div class="row">
			...
			<div class="box-body" id="report-display" style="display: none">
				...
				<table class="table table-hover table-striped" id="billing-table">
					<thead>
						<tr>
							<th class="table-sort" data-header="name" style="width: 15%">Customer</th>
							<th class="table-sort" style="width: 10%">Amount</th>
							<th class="table-sort" data-sort-method="none" style="width: 10%">Status</th>
							<th class="table-sort" data-header="or_number" style="width: 10%">OR Number</th>
...
<script>
  new Tablesort(document.getElementById('billing-table'));
</script>
...
$(document).ready(function() {
    ...
    $('#reportForm').validate({
        ...
        submitHandler: function () {
            ...
            $.ajax({
                url: "{{ url('client/crm/reports/billing') }}",
                type: 'POST',
                dataType: 'json',
                data: data,
                success: function(result){
                    ...
                    $('#header-report').html('<h4>Company Billing Report '+result.from_display+' - '+result.to_display+'</h4>');
                    $('#header-button').show();
                    ...
                    var datas = result.billings;         
            		$.each(datas, function(index, data) {
            			var i = index+1;
                    	$('#table-billing').append(
                    		'<tr>'+
                    			// '<td class="text-center">'+i+'</td>'+
                    			'<td>'+data.name+'</td>'+
                    			'<td>'+data.totalamount+'</td>'+
                    			'<td>'+data.status_display+'</td>'+
                    			'<td>'+data.latest_or_number+'</td>'+
    ...
    $('#filterbtn').click(function() {
    	...
    	$('#header-report').html('Billing Report');
    	$('#table-billing').empty();
    	$("[name='csv']").val('');
        $("[name='pdf']").val('');
